...
export default {
  ...
  run: async (client, m, args) => {
    ...
    if (new Date() - user.jointime < cooldown) {
      return m.reply(`  ✿   ࣭   ࣪  ⣹⣹   ࣭ Espera *${msToTime(nextTime - new Date())}* para volver a enviar otra invitacion.`);
    }
    if (!args || !args.length) {
      return m.reply('🧚  ㅤ＇   ❚ ❘ Ingresa el enlace para invitar al bot a tu grupo.');
    }
    const link = args.join(' ');
    const match = link.match(linkRegex);    
    if (!match || !match[1]) {
      return m.reply('   ࣪   ࣭   ⚠️   ࣪   ♡ El enlace ingresado no es válido o está incompleto.');
    }
    const isOficialBot = botId === global.client.user.id.split(':')[0] + '@s.whatsapp.net';
    const botType = isOficialBot ? 'Principal/Owner' : 'Sub Bot';
    const pp = await client.profilePictureUrl(m.sender, 'image').catch(() => 'https://files.catbox.moe/ofitow.jpeg');    
    const sugg = ` ᷼⥃ᩥ ☁️ׅ֟፝͠ᩬ⃞    ﹗ 𝗦𝗢𝗟𝗜𝗖𝗜𝗧𝗨𝗗 𝗥𝗘𝗖𝗜𝗕𝗜𝗗𝗔
...
⭐⃞░ *Versión ›* ${global.version}`;    
    ...
    if (isOficialBot) {
      ...
      for (const destino of lista) {
        try {
          await global.client.sendContextInfoIndex(destino, sugg, {}, null, false, null, { banner: pp, title: '⭐⃞░ Invitación', body: '⭐⃞░ New invitation to the Sokect.', redes: global.db.data.settings[client.user.id.split(':')[0] + "@s.whatsapp.net"].link });
        } catch {}
      }
    } else {
    ...
    await client.reply(m.chat, '  ✿   ࣭   ࣪  ⣹⣹   ࣭    El enlace fue enviado correctamente. ¡Gracias por tu invitación!  (꜆˶ᵔᵕᵔ˶)꜆', m);
