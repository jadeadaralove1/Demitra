...
export default {
  ...
  run: async (client,m,args,command,text,prefix) => {
    ...
    if (ranking.length === 0) return m.reply(`🍭⃞͝ ⚭  No hay actividad registrada en los últimos ${daysArg} días.`)

    const page = parseInt(args[1]) || 1
    const perPage = 10
    const totalPages = Math.ceil(ranking.length / perPage)
    if (page < 1 || page > totalPages) return m.reply(`(＼᳟ㅤ ⃞🤍 Página inválida. Solo hay ${totalPages} páginas disponibles.`)

    const start = (page - 1) * perPage
    const end = start + perPage
    const pageRanking = ranking.slice(start,end)

    let report = `𝄄ׄㅤ𝅄🐋 ⌒𝅄 Top de usuarios inactivos ❀\n`
    report += `>  Días: \`${daysArg}\`\n`
