...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    const text = args[0];
    if (!text) return m.reply('૮(ˊ ᵔ ˋ)ა Ingresa un enlace para realizar la solicitud.')
    if (!/^https?:\/\//.test(text)) {
      return m.reply('૮(ˊ ᵔ ˋ)ა Ingresa un enlace válido que comience con http o https');
    }
    try {
      ...
      if (contentLength > 100 * 1024 * 1024) {
        return m.reply(`૮(ˊ ᵔ ˋ)ა El archivo es demasiado grande.\nContent-Length: ${contentLength} bytes`);
      }
